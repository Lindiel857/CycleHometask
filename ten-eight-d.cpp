...
void on_draw_event (GtkWidget *widget, cairo_t *cr, gpointer data)
{
    ...
    if (N % 2 == 0) {
        for (int i = 0; i < N/2; i++) {
            cairo_move_to(cr, 100, 300 - 400.0 * (i+1) / (N+1));
            cairo_line_to(cr, 100 + 400.0 * (i+1) / (N+1), 300);
            cairo_stroke(cr);
        }
        for (int i = 0; i < N/2; i++) {
            cairo_move_to(cr, 300, 100 + 400.0 * (i+1) / (N+1));
            cairo_line_to(cr, 300 - 400.0 * (i+1) / (N+1), 100);
            cairo_stroke(cr);
        }
    }
